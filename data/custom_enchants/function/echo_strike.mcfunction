# 1. VÉRIFICATION DU COOLDOWN (100 ticks = recharge complète de la masse)
# Si le score est inférieur à 100, on arrête tout
...
execute as @e[distance=0.1..6,type=!player] at @s run damage @s 8 minecraft:sonic_boom by @p
...
execute at @s run effect give @e[distance=0.1..6,type=!player] minecraft:slowness 2 5 true
execute at @s run effect give @e[distance=0.1..6,type=!player] minecraft:slowness 4 1 true
execute at @s run effect give @e[distance=0.1..6,type=!player] minecraft:slowness 8 0 true
