...
execute as @a[scores={cooldown_masse=100..}] if items entity @s weapon.mainhand mace[enchantments~[{enchantment:"custom_enchants:echo_strike"}]] run title @s actionbar {"text":"⚡ ECHO STRIKE READY ⚡","color":"dark_aqua","bold":true}

# 3. Optionnel : Afficher une barre de chargement quand ce n'est pas prêt
# Version avec affichage du score de 0 à 100
execute as @a[scores={cooldown_masse=..99}] if items entity @s weapon.mainhand mace[enchantments~[{enchantment:"custom_enchants:echo_strike"}]] run title @s actionbar ["",{"text":"Recharge : ","color":"gray"},{"score":{"name":"@s","objective":"cooldown_masse"},"color":"white"},{"text":"/100","color":"gray"}]
...
execute as @a if items entity @s weapon.mainhand minecraft:netherite_sword[enchantments~[{enchantment:"custom_enchants:blood_line"}]] run function custom_enchants:wither/display_action_bar
...
execute as @a[scores={kills_total=1..}] if items entity @s weapon.mainhand trident[enchantments~[{enchantment:"custom_enchants:storm_harpoon"}]] run function custom_enchants:storm_harpoon/increase_charge
...
execute as @a if items entity @s weapon.mainhand trident[enchantments~[{enchantment:"custom_enchants:storm_harpoon"}]] run function custom_enchants:storm_harpoon/display_hud


# 1. Détection du trident lancé avec l'enchantement
execute as @e[type=minecraft:trident] if items entity @s contents *[enchantments~[{enchantment:"custom_enchants:storm_harpoon"}]] at @s run particle minecraft:electric_spark ~ ~ ~ 0.1 0.1 0.1 0 2
# Si un trident vole et possède l'enchantement, on lui donne le tag "is_thrown"
execute as @e[type=minecraft:trident] if items entity @s contents *[enchantments~[{enchantment:"custom_enchants:storm_harpoon"}]] run tag @s add is_thrown
...
execute as @a if items entity @s weapon.mainhand #minecraft:spears[enchantments~[{enchantment:"custom_enchants:heroic_spear"}]] run function custom_enchants:heroic_spear/display_hud



# Dans tick.mcfunction
execute as @a[scores={kill_count=20..}, x_rotation=-90..-60, predicate=minecraft:is_sneaking] if items entity @s weapon.mainhand #minecraft:spears[enchantments~[{enchantment:"custom_enchants:heroic_spear"}]] at @s run function custom_enchants:heroic_spear/trigger_arrow_rain
